...
setwd()
dat = read.csv('Dataset_ROC.csv', stringsAsFactors = T, head = T)
...
# seed                              ##
seed_range = seq(1, 5, by=1)        ##
######################################

# Ballot_Type selection
...
#############################################################################################
# performance_table
Table_range  = seed_range*length(cutoff_range)*6
performance_table = matrix(1:Table_range, nrow = length(seed_range)*length(cutoff_range), ncol = 6)
colnames(performance_table) = c('Best performance','AUC','Lowest error','Error','Cutoff','Seed')
row_position = 1
...
for (seed in seed_range)
{
  for (cutoff in cutoff_range) {
    ...
    sorted_all_err = all_ROC[order(all_ROC[,2], decreasing = F),] %>% round(rounding)
    
    #############################################################################################
    ## value assignment
    
    performance_table[row_position,'Best performance'] = rownames(sorted_all_ROC[, 0])[1]
    performance_table[row_position,'AUC'] = sorted_all_ROC[1,1]
    performance_table[row_position,'Lowest error'] = rownames(sorted_all_err[, 0])[1]
    performance_table[row_position,'Error'] = sorted_all_err[1,2]
    performance_table[row_position,'Cutoff'] = cutoff
    performance_table[row_position,'Seed'] = seed
